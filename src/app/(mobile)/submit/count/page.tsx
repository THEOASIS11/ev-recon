...
export default function PhysicalCountPage() {
  ...
  const loadExistingValues = useCallback(async () => {
    if (!existingSubmission || products.length === 0) return;

    const savedEntries: Record<string, ProductEntry> = {};
    products.forEach((p) => {
      savedEntries[p.id] = { sellable: '', unassembled: '', defective: '' };
    });

    if (existingSubmission.data?.products) {
      existingSubmission.data.products.forEach((item) => {
  ...
  const doneCount = products.filter(
    (p) => isEntryDone(entries[p.id] ?? { sellable: '', unassembled: '', defective: '' })
  ).length;
  const allDone = doneCount === products.length && products.length > 0;
  const isReadOnly = existingSubmission !== null;
    ...
    return (
      <>
        ...
        >
          ...
          <p className="s-sub" style={{ color: '#71717a', fontSize: '14px', margin: 0 }}>
            Physical count recorded and locked.
          </p>
        </div>
      </>
    );
    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .sub-btn.off { background:#27272a; color:#71717a; cursor:not-allowed; opacity: 0.5; }
      ...
      <div style={{ paddingBottom: '200px' }}>
        ...
        <div style={{ padding: '28px 16px 0' }}>
          <button
            ...
            className="back-btn"
            style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '12px', fontSize: '14px' }}
          >
          ...
          <h1 style={{ color: '#fafafa', fontSize: '22px', fontWeight: 700, margin: '0 0 6px' }}>
            Physical Count
          </h1>
          ...
          {/* Red warning banner - always shown */}
          <div
            style={{
              background: '#1a0a0a',
          ...
          {isReadOnly && (
            ...
            >
              ...
              <p style={{ color: '#93c5fd', fontSize: '13px', margin: 0, lineHeight: 1.5 }}>
                Submitted on {formatDate(existingSubmission!.submitted_at)}. This data is now locked.
              </p>
            </div>
          )}
        </div>
                ...
                >
                  ...
                  >
                    {done ? (
                      ...
                      <span style={{ color: isOpen ? '#3b82f6' : '#71717a', fontSize: '11px', fontWeight: 700 }}>
                        {idx + 1}
                      </span>
                    )}
                  </div>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {done && !isOpen && (
                      <p style={{ color: '#71717a', fontSize: '12px', margin: '3px 0 0' }}>
                        Sellable: {parseNum(e.sellable)} · Unassembled: {parseNum(e.unassembled)} · Defective:{' '}
                        {parseNum(e.defective)}
                      </p>
                    )}
                  </div>
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    {done ? (
                      <span style={{ color: '#3b82f6', fontSize: '16px', fontWeight: 700 }}>{total}</span>
                    ) : (
                      <svg
                        width="16"
                        height="16"
                ...
                <div className={`acc-body${isOpen ? ' open' : ''}`}>
                  <div style={{ padding: '0 16px 20px' }}>
                    ...
                    {isReadOnly ? (
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        <div>
                          ...
                          <div className="pc-display">{parseNum(e.sellable)}</div>
                        </div>
                        <div>
                          <label className="field-label">Unassembled</label>
                          <div className="pc-display">{parseNum(e.unassembled)}</div>
                        </div>
                        <div>
                          <label className="field-label">Defective</label>
                          <div className="pc-display">{parseNum(e.defective)}</div>
                        </div>
                      </div>
                    ) : (
                      // Editable inputs
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '14px' }}>
                      <span style={{ color: '#3b82f6', fontSize: '16px', fontWeight: 700 }}>Total: {total}</span>
                      {!isReadOnly && idx < products.length - 1 && (
                        <button
      ...
      {!isReadOnly && (
        ...
        >
          <p style={{ color: '#71717a', fontSize: '13px', margin: '0 0 10px', textAlign: 'center' }}>
            {allDone
          ...
          >
            {loading ? 'Submitting...' : 'Submit Physical Count'}
          </button>
        </div>
      )}
    </>
